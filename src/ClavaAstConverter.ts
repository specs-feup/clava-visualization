...
import GenericAstConverter, {
  FilesCode,
} from "@specs-feup/lara-visualization/api/GenericAstConverter.js";
import ToolJoinPoint, {
  JoinPointInfo,
} from "@specs-feup/lara-visualization/api/public/js/ToolJoinPoint.js";
import {
  AccessSpecifier,
  AdjustedType,
  Body,
...
export default class ClavaAstConverter implements GenericAstConverter {
  ...
  private refineCode(node: CodeNode, indentation: number = 0): CodeNode {
    if (node.code) node.code = addIdentation(node.code, indentation);
    ...
    for (const child of node.children) {
      const newIndentation =
        node.jp instanceof Scope || node.jp instanceof Class
          ? indentation + 1
          : indentation;
    ...
    if (
      ...
      node.children[0].jp.astName === "TagDeclVars"
    ) {
  ...
  private syntaxHighlight(
    ...
    if (node.jp instanceof Declarator || node.jp instanceof EnumeratorDecl) {
      ...
      const regex = new RegExp(
        `\\s*(&amp;|\\*)*\\s*` + (escapedName ? `\\b${escapedName}\\b` : "$")
      ); // Match the declaration name (if one exists) with the '&' and '*' prefixes
      const namePos = code.search(regex);
      return namePos <= 0
        ? code
        : openingTag +
            code.slice(0, namePos) +
            closingTag +
            code.slice(namePos);
    }

    if (
      node.jp instanceof Statement ||
      node.jp instanceof Decl ||
      node.jp instanceof Expression
    ) {
      const [openingTag, closingTag] = getSyntaxHighlightTags("keyword");

      if (
        node.jp instanceof Switch ||
        node.jp instanceof Break ||
        node.jp instanceof Case ||
        node.jp instanceof Continue ||
        node.jp instanceof GotoStmt ||
        node.jp instanceof ReturnStmt ||
        node.jp instanceof EnumDecl ||
        node.jp instanceof AccessSpecifier ||
        node.jp instanceof DeleteExpr ||
        [
          "FunctionTemplateDecl",
          "TemplateTypeParmDecl",
          "NamespaceDecl",
        ].includes(node.jp.astName)
      ) {
      ...
      if (node.jp instanceof If) {
        const elseMatch = code.match(
          /^(([^/]|\/[^/*]|\/\/.*|\/\*([^*]|\*[^/])*\*\/)*?)(?<!>)\belse\b/
        ); // Match first unhighlighted 'else' that is not inside a comment (single or multi-line)
        ...
        if (elseMatch) {
          ...
          return (
            ...
            "if" +
            closingTag +
            code.slice(2, elsePos) +
            openingTag +
            "else" +
            closingTag +
            code.slice(elsePos + 4)
          );
        } else {
      ...
      if (node.jp instanceof Loop) {
        if (node.jp.kind == "dowhile") {
          const whilePos = code.match(
            /^(([^/]|\/[^/*]|\/\/.*|\/\*([^*]|\*[^/])*\*\/)*?)(?<!>)\bwhile\b/
          )![1].length; // Match first unhighlighted 'while' that is not inside a comment (single or multi-line)
          ...
          return (
            ...
            "do" +
            closingTag +
            code.slice(2, whilePos) +
            openingTag +
            "while" +
            closingTag +
            code.slice(whilePos + 5)
          );
        } else {
          return code.replace(/^(\w+)\b/, `${openingTag}$1${closingTag}`); // Highlight first word
        }
      ...
      if (node.jp instanceof TypedefDecl && node.code!.startsWith("typedef")) {
        // The code of a TypedefDecl can be divided, and the second part does not have the keyword
      ...
      if (node.jp instanceof RecordJp) {
        return code.replace(
          /(class(?!=)|struct)/,
          `${openingTag}$1${closingTag}`
        ); // Highlight 'class' or 'struct' in declaration
      }
   ...
   * linked code.
   */
  private linkCodeToAstNodes(
    root: CodeNode,
    outerCode: string | undefined,
    outerCodeStart: number,
    outerCodeEnd: number
  ): any[] {
    const nodeCode = root.code;
    if (!nodeCode || !outerCode) return [outerCodeStart, outerCodeStart, ""]; // Return empty string if the node has no code
    ...
    if (innerCodeStart === -1 || innerCodeEnd > outerCodeEnd) {
      console.warn(
        `Code of node "${root.jp.joinPointType}" not found in code container: "${nodeCodeHtml}"`
      );
    ...
    if (root.jp instanceof Vardecl) {
      newCodeIndex =
        outerCode.slice(innerCodeStart, innerCodeEnd).search(/[=;]/) +
        innerCodeStart +
        1;
    ...
    for (const child of root.children) {
      const [childCodeStart, childCodeEnd, childCode] = this.linkCodeToAstNodes(
        child,
        outerCode,
        newCodeIndex,
        innerCodeEnd
    ...
    if (root instanceof Program) {
      return Object.fromEntries(
        rootCodeNode.children.map((child) => {
          ...
          const fileLinkedHtmlCode = this.linkCodeToAstNodes(
            child,
            fileHtmlCode,
            0,
            fileHtmlCode.length
